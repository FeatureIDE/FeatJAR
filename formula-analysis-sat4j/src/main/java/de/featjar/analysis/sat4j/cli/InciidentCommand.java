...
public class InciidentCommand extends ASAT4JAnalysisCommand<BooleanAssignmentList> {
     ...
     */
    public static final AOption<Integer> LIMIT_OPTION = Options.newOption("n", Options.IntegerParser) //
            .setDescription("Maximum number of tests to be performed.") //
            .setDefaultValue(Integer.toString(Integer.MAX_VALUE));
     ...
     */
    public static final AOption<Path> INITIAL_SAMPLE_OPTION = Options.newOption("initial-sample", Options.PathParser)
     ...
     * Path option for initial sample.
     */
    public static final AOption<Path> TESTER_EXECUTABLE = Options.newOption("verifier", Options.PathParser)
            .setDescription("Path to initial sample file.")
            .setValidator(Options.PathValidator);
