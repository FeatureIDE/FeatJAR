...
/**
 * Computes the count of each {@link ATerminalExpression terminal} node (variables, constants,...).
 *
 * @author Mohammad Khair Almekkawi
 * @author Florian Beese
 * @author Sebastian Krieter
 * */
public class ComputeConstraintNumberOfAtoms extends AFeatureModelComputation<DataTree<Long>> {

    public ComputeConstraintNumberOfAtoms(IComputation<IFeatureModel> featureModel) {
        super(featureModel);
    }
        ...
        for (IConstraint constraint : FEATURE_MODEL.get(dependencyList).getConstraints()) {
            counts.add(DataTree.ofMap(
                    ...
                    Trees.preOrderStream(constraint.getFormula())
                            .filter(n -> n instanceof ATerminalExpression)
                            .collect(Collectors.groupingBy(IExpression::getName, Collectors.counting())),
                    Long::sum,
                    () -> 0L));
        }
        return Result.of(DataTree.ofAggregator("AtomCount", 0L, Long::sum, counts));
    }
}
