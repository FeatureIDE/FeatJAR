...
import de.featjar.gui.operation.SetCardinalityFeatureBoundsOperation;
...
/**
 * The handler sets the {@link Cardinality} bounds of a {@link Feature}
 * and is triggered by the underlying operation {@link SetCardinalityFeatureBoundsOperation}.
 */
public class SetCardinalityFeatureBoundsHandler extends GModelOperationHandler<SetCardinalityFeatureBoundsOperation> {

    @Inject
    protected IdentifiableResolver resolver;

    @Override
    public Optional<Command> createCommand(SetCardinalityFeatureBoundsOperation operation) {
        ...
        if ((upper < lower && upper != CardinalityUtils.OPEN) || lower < 0) {
            return Optional.empty();
        }

        return commandOf(() -> {
            Cardinality c = feature.getCardinality();
